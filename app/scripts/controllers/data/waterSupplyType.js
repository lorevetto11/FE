...
angular.module('APP')
	.controller('WaterSupplyType', function ($scope, $state, $translate, $q, $uibModal, $log, $document, $window,
	                                        currentUser, currentOrganization, Shape, WaterSupplyType, RelatedFrequency, ModalService, APIService, ResourceService, StorageService, ValidationService, notify, UtilsService, PermissionService) {
		...
		$scope.add = function () {
			$scope.selectedItem = new WaterSupplyType();
		...
		$scope.delete = function (item) {
			...
			ModalService.dialogConfirm("alertMessage.delete", ' "entity.type" <strong>' + item.name + '</strong> "alertMessage.willBeDeleted"',
				function onConfirmAction() {
					...
					return APIService.deleteWaterSupplyType(item.id).then(function success(){}, savingError);
			...
			if (item.id == null) {
				...
				APIService.createShape(item.shape).then(
					function success(data) {
						...
						APIService.createWaterSupplyType(item).then(
				...
				APIService.updateShape(item.shape).then(
					function success() {
						...
						APIService.updateWaterSupplyType(item).then(
							function success(item) {
		...
		function init() {
			...
			APIService.getWaterSupplyTypesByOrganizationId($scope.selectedOrganization.id).then(
