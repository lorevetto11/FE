...
angular.module('APP')
	.controller('WasteDisposalType', function ($scope, $state, $translate, $q, $uibModal, $log, $document, $window,
	                                           currentUser, currentOrganization, Shape, WasteDisposalType, RelatedFrequency, ModalService, APIService, ResourceService, StorageService, ValidationService, notify, UtilsService, PermissionService) {
		...
		$scope.add = function () {
			$scope.selectedItem = new WasteDisposalType();
		...
		$scope.delete = function (item) {
			...
			ModalService.dialogConfirm("alertMessage.delete", ' "entity.type" <strong>' + item.name + '</strong> "alertMessage.willBeDeleted"',
				function onConfirmAction() {
					...
					return APIService.deleteWasteDisposalType(item.id).then(function success() {
					}, savingError);
			...
			if (item.id == null) {
				...
				APIService.createShape(item.shape).then(
					function success(data) {
						...
						APIService.createWasteDisposalType(item).then(
				...
				APIService.updateShape(item.shape).then(
					function success() {
						...
						APIService.updateWasteDisposalType(item).then(
							function success(item) {
		...
		function init() {
			...
			APIService.getWasteDisposalTypesByOrganizationId($scope.selectedOrganization.id).then(
